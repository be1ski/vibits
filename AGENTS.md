...
- **Use `@Inject` when mocking is needed.** Add `@Inject` for classes with side effects (network, DB, files) or when you need test isolation. Pure functions can skip DI if testing them directly is simpler.
