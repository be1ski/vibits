...
class OnboardingReducerTest {
  ...
  fun `when HabitCreated then moves to Success step`() =
    onboardingReducer.test(
      ...
      assertHasEffect<OnboardingEffect.Notification.HabitCreatedSuccessfully>()
    }
  ...
  @Test
  fun `when Skip then emits Skipped notification and marks completed`() =
    onboardingReducer.test(OnboardingState()) {
  ...
  @Test
  fun `when MarkFirstCheckIn then marks check-in, completes onboarding, and emits Completed`() =
    onboardingReducer.test(OnboardingState(currentStep = OnboardingStep.Success)) {
