  ...
  private val testReducer: Reducer<Action, State, Effect, Nothing> =
    reducer { action, state ->
      when (action) {
        ...
        Action.Decrement -> state { state.copy(count = state.count - 1) }
        Action.StartLoading -> {
          state { state.copy(loading = true) }
          command(Effect.Load)
        }
        Action.LoadingComplete -> state { state.copy(loading = false) }
        is Action.SetCount -> state { state.copy(count = action.value) }
      }
    }
